...
sub data {
  ...
  if($outofstock) {
    pg_cmd q{UPDATE extlinks SET deadsince = NULL, price = '', lastfetch = NOW() WHERE site = 'denpa' AND value = $1}, [ $id ];
    AE::log debug => "$prefix is out of stock";

  # We have a price? Update database.
  } elsif($price) {
    pg_cmd q{UPDATE extlinks SET deadsince = NULL, price = $2, lastfetch = NOW() WHERE site = 'denpa' AND value = $1}, [ $id, $price ];
    AE::log debug => "$prefix for $price";

  # Not found? Update database.
  } else {
    pg_cmd q{UPDATE extlinks SET deadsince = NOW() WHERE deadsince IS NULL AND site = 'denpa' AND value = $1}, [ $id ];
...
sub sync {
  pg_cmd "SELECT value FROM extlinks WHERE site = 'denpa' AND c_ref ORDER BY lastfetch ASC NULLS FIRST LIMIT 1", [], sub {
    my($res, $time) = @_;
    return if pg_expect $res, 1 or !$res->nRows;

    my $id = $res->value(0,0);
    my $ts = AE::now;
    http_get sprintf($VNDB::ExtLinks::LINKS{denpa}{fmt}, $id),
